...
public class BroadcastDataParser {
    ...
    public static BaseDevice parsePackage(DatagramPacket packet) throws UnknownHostException {
        ...
        BaseDevice mDevice = null;
        if (BroadcastDataParser.isValid(receiveContent)) {
            String deviceTypeStr = BroadcastDataParser.filterType(receiveContent);
            if (deviceTypeStr == null) {
                Log.d(TAG, " type is null or the type of the device don't support local mode.");
            } else {
                hostname = BroadcastDataParser.filterIpAddress(receiveContent);
                if (hostname.equals("0.0.0.0")) {
                    return mDevice;
                }
                responseAddr = InetAddress.getByName(hostname);
                Log.d(TAG, receiveContent);
                responseBSSID = BroadcastDataParser.filterBssid(receiveContent);
                TYPE type = TYPE.getTypeEnumByString(deviceTypeStr);
                mDevice = new BaseDevice(type,  responseAddr.getHostAddress(), responseBSSID);
            }
        }
        return mDevice;
    }
